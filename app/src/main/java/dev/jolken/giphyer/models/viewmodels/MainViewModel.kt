...
class MainViewModel : BaseViewModel() {
    ...
    fun loadGifs() {
        requestWithCallback({
            if (searchInputText.value != null && searchInputText.value?.length == 0) api.getTrending(
                TrendingRequest(
                    API_KEY, 20, requestGetGifsOffset.value ?: 0
                )
            )
            else api.getSearch(SearchRequest(API_KEY, searchInputText.value ?: "", 20,requestGetGifsOffset.value ?: 0))
        }) {
            when (it.status) {
                Status.ERROR -> {
                ...
                Status.SUCCESS -> {
                    gifsList.value = Event(it.status, gifsList.value?.data?.apply {
                        if (it.data?.data != null) this.addAll(it.data.data)
                    }, null)
                }
            }
        }
